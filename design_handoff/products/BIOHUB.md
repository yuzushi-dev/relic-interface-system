...
> **Note:** adopting RIS here is a deliberate **redesign** from the app's native Material 3 look (stakeholder-confirmed). For the Kotlin port of these tokens + components, see `../design/compose/`.

## Run it
Open `../design/ui_kits/biohub/index.html` (React + Babel + Lucide + `charts.js` from CDN/local; no build). A scaled phone bezel hosts a bottom-tab app.
...
| `ui/theme/*` (Material 3) | replaced by RIS tokens; Kotlin port + pre-written composables in `../design/compose/` (incl. `BioHubScreens.kt`) |
