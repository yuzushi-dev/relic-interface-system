...
- **Kiroshi Tactical Scrubber & Real Telemetry (`ris-charts.js`)**:
...
- **JSON Token Alignment (`ris.tokens.json`)**: introduced `skin.cyber` branch in official design tokens to explicitly model the Cyberpunk 2077 palette (dark crimson surfaces, structural red lines, saturated accents, Chakra Petch font).
...
- **Skin `data-skin="cyber"`** (`css/ris-skin-cyber.css`): Cyberpunk 2077 look & feel as an **opt-in layer**, loaded last over the forensic baseline. Dark mode only. Features:
...
- **Faction themes** `data-brand="arasaka|militech|edgerunners"`: full case-context skins (accent + tinted surfaces in dark, AA ink in light). Verified against CP2077 lore and artbooks.
...
### Breaking
- Palette aligned with Cyberpunk 2077: yellow `#fcee0a`, cyan `#00f0ff`, red `#ff003c` (fill) / `#ff4066` (dark contextual), green `#00e57e`, violet `#9a5cff`, magenta `#ff42c8`, orange `#ff9230`.
